...
export const Timer = ({initial}) => {
  ...
  return (
    ...
    <h3 className={styles.text_div}>
      ...
      {(`${Math.floor(time/ 60)%60}` > 0) ? <span className={styles.s_text}>{`${Math.floor(time/60)%60}`}<span style={{color:"white",fontSize:"30px",fontWeight:"lighter"}}>m</span></span> : null }
      <span className={styles.s_text}>{`${Math.floor(time%100)}`}<span style={{color:"white",fontSize:"30px",fontWeight:"lighter"}}>s</span></span></h3>
